...
Context::Context(int rank, int size, int dev_id, int base)
    : ::gloo::Context(rank, size, base), dev_id(dev_id) {}

Context::~Context() {}

void Context::connectFullMesh(sophgo::Store& store,
                              std::shared_ptr<transport::Device>& dev) {
  auto transportContext = dev->createContext(rank, size);
  transportContext->setDevId(dev_id);
...
ContextFactory::ContextFactory(std::shared_ptr<::gloo::Context> backingContext)
  ...
  for (auto i = 0; i < backingContext_->size; i++) {
    ...
    try {
      GLOO_ENFORCE(backingContext_->getPair(i) != nullptr,
...
std::shared_ptr<::gloo::Context> ContextFactory::makeContext(
    std::shared_ptr<transport::Device>& dev) {
  auto context =
      std::make_shared<Context>(backingContext_->rank, backingContext_->size, 0); // default dev_id is 0, how to modify?

...
#include <gloo/transport/sophgo/attr.h>
#include <gloo/transport/sophgo/error.h>
#include <gloo/transport/sophgo/listener.h>
#include <gloo/transport/sophgo/loop.h>
#include <gloo/transport/sophgo/socket.h>
...
//   virtual std::shared_ptr<::gloo::transport::Context> createContext(int rank,
//                                                                     int size,
//                                                                     int dev_id);
 ...
 protected:
  ...
  void connect(const Address& local, const Address& remote,
               std::chrono::milliseconds timeout, connect_callback_t fn);

  void connectAsListener(const Address& local,
                         std::chrono::milliseconds timeout,
                         connect_callback_t fn);

  void connectAsInitiator(const Address& remote,
                          std::chrono::milliseconds timeout,
                          connect_callback_t fn);

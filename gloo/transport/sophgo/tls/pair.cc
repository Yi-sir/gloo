...
#include "gloo/transport/sophgo/tls/pair.h"

#include "gloo/common/error.h"
#include "gloo/common/logging.h"
#include "gloo/transport/sophgo/buffer.h"
#include "gloo/transport/sophgo/tls/context.h"
#include "gloo/transport/sophgo/tls/device.h"
...
Pair::Pair(Context *context, Device *device, int rank,
    ...
    : ::gloo::transport::sophgo::Pair(context, device, rank, timeout),
      ssl_(nullptr),
...
void Pair::handleReadWrite(int events) {
  if (!is_ssl_connected_ && !device_->isInitiator(self_, peer_)) {
    ...
    sophgo::Pair::handleReadWrite(events);
  }
}
...
void Pair::changeState(Pair::state nextState) noexcept {
  ...
  ::gloo::transport::sophgo::Pair::changeState(nextState);
}
...
void Pair::waitUntilConnected(std::unique_lock<std::mutex> &lock,
                              bool useTimeout) {
  ::gloo::transport::sophgo::Pair::waitUntilConnected(lock, useTimeout);
...
void Pair::verifyConnected() {
  ::gloo::transport::sophgo::Pair::verifyConnected();

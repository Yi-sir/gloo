...
ssize_t Pair::prepareWrite(
  ...
  if (op.nwritten < sizeof(op.preamble)) { 
    // 默认nwritten是0
    // iov[0]的起始地址是op.preamble的地址，长度是op.preamble的长度
  ...
  // Send data to a remote buffer, example走这个
  if (opcode == Op::SEND_BUFFER) {
    // iov[1]的起始地址是op.buf的地址
    char* ptr = (char*)op.buf->ptr_;
    size_t offset = op.preamble.offset; // 默认是0
    size_t nbytes = op.preamble.length; // 写入数据的长度
    if (op.nwritten > sizeof(op.preamble)) {
...
bool Pair::write(Op& op) {
  ...
  const auto opcode = op.getOpcode(); // SEND_BUFFER
...
void Pair::readComplete(NonOwningPtr<UnboundBuffer> &buf) {
  ...
  switch (opcode) {
    case Op::SEND_BUFFER:// example走这里

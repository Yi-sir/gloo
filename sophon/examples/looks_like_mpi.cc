...
// Global context
std::shared_ptr<sophon::Context> kContext;
...
int MPI_Barrier(MPI_Comm comm) {
  ASSERT(comm == MPI_COMM_WORLD);
  sophon::BarrierOptions opts(kContext);
  sophon::barrier(opts);
}
    ...
    MPI_Comm comm)
{
  ...
  ASSERT(op == MPI_SUM);
  ASSERT(comm == MPI_COMM_WORLD);
  sophon::AllreduceOptions opts(kContext);
...
void init(const std::string& path) {
  ...
  // Initialize store
  auto fileStore = sophon::rendezvous::FileStore(path);

...
#include "sophon/transport/uv/device.h"
...
int main(void) {
  ...
  // The following statement creates a TCP "device" for Sophon to use.
  // See "sophon/transport/device.h" for more information. For the
  // purposes of this example, it is sufficient to see the device as
  // a factory for every communication pair.
  //
  // The argument to sophon::transport::tcp::CreateDevice is used to
  ...
  //
  //   auto dev = sophon::transport::tcp::CreateDevice("localhost");
  //

  auto dev = sophon::transport::uv::CreateDevice(attr);
  ...
  //
  // See "sophon/rendezvous/store.h" for the functionality you need to
  ...
  //
  auto fileStore = sophon::rendezvous::FileStore("/libtmp");
  ...
  auto prefixStore = sophon::rendezvous::PrefixStore(prefix, fileStore);

  // Using this store, we can now create a Sophon context. The context

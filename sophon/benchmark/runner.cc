...
Runner::Runner(const options& options) : options_(options) {
  ...
  SOPHON_ENFORCE(contextFactory_, "No means for rendezvous");
...
void Runner::rendezvousMPI() {
  ...
  SOPHON_ENFORCE_EQ(rv, MPI_SUCCESS);
  MPI_Comm_rank(MPI_COMM_WORLD, &options_.contextRank);
  MPI_Comm_size(MPI_COMM_WORLD, &options_.contextSize);
  auto backingContext = std::make_shared<::sophon::mpi::Context>(MPI_COMM_WORLD);
...
void Runner::printDistribution(
  ...
  SOPHON_ENFORCE_GE(latency.size(), 1, "No latency samples found");

...
#include "sophon/allreduce_bcube.h"
...
#include "sophon/reduce_scatter.h"
#include "sophon/scatter.h"
#include "sophon/common/aligned_allocator.h"
#include "sophon/common/common.h"
#include "sophon/common/logging.h"
#include "sophon/context.h"
#include "sophon/types.h"

#include "sophon/benchmark/benchmark.h"
#include "sophon/benchmark/runner.h"

using namespace sophon;
using namespace sophon::benchmark;
...
class AllgatherRingBenchmark : public Benchmark<T> {
 ...
 public:
  void initialize(size_t elements) override {
    auto inPtrs = this->allocate(this->options_.inputs, elements);
    SOPHON_ENFORCE_EQ(inPtrs.size(), this->options_.inputs);
...
#define RUN_BENCHMARK(T)                                                   \
  ...
  if (x.benchmark == "allgather") {                                        \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllgatherBenchmark<T>>(context, x);         \
    };                                                                     \
  } else if (x.benchmark == "allgather_v") {                               \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllgathervBenchmark<T>>(context, x);        \
    };                                                                     \
  } else if (x.benchmark == "allgather_ring") {                            \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllgatherRingBenchmark<T>>(context, x);     \
    };                                                                     \
  } else if (x.benchmark == "allreduce_ring") {                            \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllreduceBenchmark<AllreduceRing<T>, T>>(   \
          context, x);                                                     \
    };                                                                     \
  } else if (x.benchmark == "allreduce_ring_chunked") {                    \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<                                            \
    ...
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllToAllBenchmark<T>>(context, x);          \
    };                                                                     \
  } else if (x.benchmark == "alltoall_v") {                                \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<AllToAllvBenchmark<T>>(context, x);         \
    };                                                                     \
  } else if (x.benchmark == "barrier_all_to_all") {                        \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<BarrierAllToAllBenchmark<T>>(context, x);   \
    };                                                                     \
  } else if (x.benchmark == "barrier_all_to_one") {                        \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<BarrierAllToOneBenchmark<T>>(context, x);   \
    };                                                                     \
  } else if (x.benchmark == "broadcast") {                                 \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<BroadcastBenchmark<T>>(context, x);         \
    };                                                                     \
  } else if (x.benchmark == "broadcast_one_to_all") {                      \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<BroadcastOneToAllBenchmark<T>>(context, x); \
    };                                                                     \
  } else if (x.benchmark == "pairwise_exchange") {                         \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<PairwiseExchangeBenchmark<T>>(context, x);  \
    };                                                                     \
  } else if (x.benchmark == "reduce") {                                    \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<ReduceBenchmark<T>>(context, x);            \
    };                                                                     \
  } else if (x.benchmark == "reduce_scatter") {                            \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<ReduceScatterBenchmark<T>>(context, x);     \
    };                                                                     \
  } else if (x.benchmark == "scatter") {                                   \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<ScatterBenchmark<T>>(context, x);           \
    };                                                                     \
  } else if (x.benchmark == "sendrecv_roundtrip") {                        \
    SOPHON_ENFORCE_EQ(x.contextSize, kSendRecvProcesses,                     \
      kNumProcessesErrorString, x.contextSize);                            \
    fn = [&](std::shared_ptr<Context>& context) {                          \
      return sophon::make_unique<SendRecvRoundtripBenchmark<T>>(context, x); \
    };                                                                     \
  ...
  if (!fn) {                                                               \
    SOPHON_ENFORCE(false, "Invalid algorithm: ", x.benchmark);               \
  }                                                                        \
  Runner r(x);                                                             \
  r.run(fn);
...
void runNewBenchmark(options& options) {
  ...
  if (name == "allreduce_ring") {
    fn = [&](std::shared_ptr<Context>& context) {
      return sophon::make_unique<NewAllreduceBenchmark<T>>(context, options);
    };
  } else {
    SOPHON_ENFORCE(false, "Invalid benchmark name: ", options.benchmark);
  }

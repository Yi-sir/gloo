...
void create_ca_pkey_cert(const std::string& _ca_pkey_file,
  ...
  SOPHON_ENFORCE(0 == std::system((std::string("bash -c \'") +
               "openssl genrsa -out " + _ca_pkey_file + " 2048" + debug + _and +
               "openssl req -subj \"/C=US/ST=New York/L=New York/O=Sophon Certificate Authority\" -new -x509 -days 1 -key " + _ca_pkey_file + " -sha256 -extensions v3_ca -out " + _ca_cert_file + debug +
               "\'").c_str()), "Could not create CA private key and certificate");
}
...
void create_pkey_cert(const std::string& _cert_file,
  ...
  SOPHON_ENFORCE(0 == std::system((std::string("bash -c \'") +
                                 "openssl genrsa -out " + _pkey_file + " 2048" + debug + _and +
                                 "openssl req -subj \"/C=US/ST=California/L=San Francisco/O=Sophon Testing Company\" -sha256 -new -key " + _pkey_file + " -out " + csr_file + debug + _and +
                                 "openssl x509 -sha256 -req -in " + csr_file + " -CA " + _ca_cert_file + " -CAkey " + _ca_pkey_file + " -CAcreateserial -out " + _cert_file + " -days 1" + debug + _and +
                                 "openssl verify -CAfile " + _ca_cert_file + " " + _cert_file + debug +
                                 "\'").c_str()), "Could not create private key and certificate");
}

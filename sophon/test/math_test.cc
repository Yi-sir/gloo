...
TYPED_TEST(MathTest, Sum) {
  ...
  for (auto& fn : reorderFunctionArguments(&sophon::sum<TypeParam>)) {
...
TYPED_TEST(MathTest, Product) {
  std::array<TypeParam, num> a, b, c;
  for (auto& fn : reorderFunctionArguments(&sophon::product<TypeParam>)) {
    for (auto i = 0; i < num; i++) {
      for (auto j = 0; j < num; j++) {
        a[j] = 2;
        b[j] = 2;
...
TYPED_TEST(MathTest, Min) {
  std::array<TypeParam, num> a, b, c;
  for (auto& fn : reorderFunctionArguments(&sophon::min<TypeParam>)) {
    for (auto i = 0; i < num; i++) {
      for (auto j = 0; j < num; j++) {
        a[j] = 2;
        b[j] = 3;
...
TYPED_TEST(MathTest, Max) {
  std::array<TypeParam, num> a, b, c;
  for (auto& fn : reorderFunctionArguments(&sophon::max<TypeParam>)) {
    for (auto i = 0; i < num; i++) {
      for (auto j = 0; j < num; j++) {
        a[j] = 2;
        b[j] = 3;
...
TYPED_TEST(MathTest, SumPerf) {
  perf<TypeParam>(&sophon::sum<TypeParam>);
}

TYPED_TEST(MathTest, ProductPerf) {
  perf<TypeParam>(&sophon::product<TypeParam>);
}

TYPED_TEST(MathTest, MinPerf) {
  perf<TypeParam>(&sophon::min<TypeParam>);
}

TYPED_TEST(MathTest, MaxPerf) {
  perf<TypeParam>(&sophon::max<TypeParam>);
}

...
void reduce(ReduceOptions& opts) {
  ...
  for (int i = 0; i < chips2260; ++i) {
    // call sgdnn_c2c_reduce
    printf("Backend SOPHON is running!\n");
  }
  return ;
}
...
//   SOPHON_ENFORCE(opts.reduce != nullptr);
...
//   SOPHON_ENFORCE_EQ(in->size, opts.elements * opts.elementSize);
//   SOPHON_ENFORCE_EQ(out->size, opts.elements * opts.elementSize);
